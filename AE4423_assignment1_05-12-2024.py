...
for i in N:
    for j in N:
        for k in K:
            if d[i, j] > R[k] or RW_AC[k] > RW_AP[i]:
                continue  # Skip infeasible pairs early
            z[i, j, k] = m.addVar(lb=0, vtype=GRB.INTEGER)
...
#     m.addConstr(
#         quicksum(
#             (d[i][j] / sp[k] + TAT[k] * (1.5 if j == hub else 1)) * z[i, j, k]
...
for k in K:
    m.addConstr(
        gp.quicksum(
            (d[i, j] / sp[k] + TAT[k] * (1.5 if Airports[j] == 'EDDF' else 1)) * z[i, j, k]
            for i in range(n) for j in range(n) if (i != j and (i, j, k) in z)
        ) <= BT * AC[k],
