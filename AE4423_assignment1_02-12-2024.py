...
TAT_hub = [float(tat) * 1.5 for tat in TAT_nohub]
...
for k in K:
    if j == hub:
        m.addConstr(quicksum(quicksum((d[i][j] / sp[k] + TAT_hub[k]) * z[i, j, k] for i in N) for j in N) <= BT * AC[k], 
                name="TimeConstraint")  # C4
    else:
        m.addConstr(quicksum(quicksum((d[i][j] / sp[k] + TAT_nohub[k]) * z[i, j, k] for i in N) for j in N) <= BT * AC[k], 
                name="TimeConstraint")  # C4
