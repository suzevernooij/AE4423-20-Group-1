...
# Check paths with at least one flight exceeding capacity
...
for p in P_i:
    ...
    bpr[(422, 422)] = 0 # adds a recpature rate of 0 for reallocations from fictitious tot fictitious
#Dp.append(1000) # sets a high limit of demand for the fictitious itinerary 
...
model_status_initial = m2.status

if model_status_initial == GRB.Status.UNBOUNDED:
    print('The model cannot be solved because it is unbounded')

elif model_status_initial == GRB.Status.OPTIMAL or True:
    f_objective_initial = m2.objVal
    print('--------------------- Initial value results----------------------')
    print('\nObjective Function Value Initial value: \t %g' % f_objective_initial)
    print('\n----------------------------------------------------------------------------------------------------\n')

#%%                                                  ----- Printing initial KPI's -----
t_values = []
print('\n The t[p,r] values for the initial value')
for key, var in t.items():
    if var.X != 0:
        t_values.append((key, var.X))

print("Values from t_values:")
...
while not terminate_iteration and iteration_count < 20:
    ...
    reduced_costs = {(p, r): (fare[p] - sum(dual_CAP[i] for i in itinerary_dict[p]) 
                                - bpr.get((p, r), 0) * (fare[r] - sum(dual_CAP[i] * itinerary_dict[r])) 
    ...
    for n in negative_columns:
        ...
        for b in itinerary_dict[n[0]]:
            new_column.addTerms((delta_matrix[b][n[0]] - delta_matrix[b][n[1]] * bpr.get((n[0], n[1]), 0)), m2.getConstrByName('CAP'%(b)))
        new_column.addTerms(1, m2.getConstrByName("D"%(n[0])))

        t[n[0], n[1]] = m2.addVar(obj=fare[n[0]] - bpr.get((n[0], n[1]), 0) * fare[n[1]], lb=0, vtype=GRB.INTEGER, column=new_column)
        added_columns.append((n[0], n[1]))
        columns.append((n[0], n[1]))
